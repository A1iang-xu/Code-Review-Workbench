...
async def test_run_review_pipeline_success():
    ...
    with patch.object(tasks, "review_graph", MagicMock()) as mock_graph, \
         ...
         patch("app.core.orchestrator.update_progress") as _mock_update, \
         patch("app.core.orchestrator.complete_progress") as _mock_complete:
...
async def test_run_review_pipeline_graph_failure():
    ...
    with patch.object(tasks, "review_graph", MagicMock()) as mock_graph, \
         patch.object(tasks, "_save_to_db", AsyncMock()) as mock_save, \
         patch.object(tasks, "fail_progress") as mock_fail, \
         patch("app.core.orchestrator.update_progress") as _mock_update2, \
         patch("app.core.orchestrator.complete_progress") as _mock_complete2:
        # ainvoke 抛异常
