...
class RefactorAdvisorAgent(BaseReviewAgent):
    ...
    def _count_params(self, func_node) -> int:
        ...
        Tree-sitter Python:
        function_definition: 'def' name parameters ...
        parameters: '(' ... parameter* ... ')'

        Args:
            func_node: function_definition AST 节点
            ...
            if child.type == "parameters":
                for param in child.children:
                    if param.type in ("identifier", "typed_parameter", "typed_default_parameter",
                                      "default_parameter", "list_splat_pattern",
    ...
    async def _llm_refactor_advice(self, parsed_file: ParsedFile) -> list[dict]:
        ...
        try:
            response = await self._llm_analyze(
                prompt=(
                    f"{REFACTOR_PROMPT}\n\n"
                    f"Analyze the following Python file for code smells. "
